...
class TradingDaemon:
    ...
    def handle_telegram_command(self, cmd: str) -> str:
        ...
        if cmd == "/status":
            ...
            for p in open_pos:
                lines.append(
                    f"• <b>{p['symbol']}</b> ({p['direction']}) | Qty: {p['qty']}\n"
            ...
            return (
                ...
                f"🎯 Win Rate: {wr}%\n"
                f"💰 Total Realized P&L: <b>₹{pnl:+.2f}</b>"
            )
        ...
        return "Commands: /status, /positions, /pnl, /scan"
    ...
    def run(self):
        """Main daemon loop."""
        print("==================================================")
        print("   PROJECT ATLAS — AUTONOMOUS TRADING DAEMON      ")
        print(f"   Universe: {len(NSE_UNIVERSE)} Stocks | Mode: {self.mode}     ")
        print(f"   Telegram Alerts: {'ACTIVE' if self.notifier.is_enabled else 'NOT CONFIGURED'}")
        print("==================================================")

        while self.is_running:
            try:
                # Check incoming Telegram commands
                self.notifier.check_incoming_commands(self.handle_telegram_command)

                now_ist = datetime.now(IST).time()
                weekday = datetime.now(IST).weekday()

                # Market hours: 9:15 AM - 3:30 PM IST, Monday (0) to Friday (4)
                ...
                if is_weekday and is_market_hours:
                ...
                elif is_weekday and now_ist >= SQUARE_OFF and not self.daily_report_sent:
                    # Square off and send daily report
                    state = self.load_state()
                    ...
                    # Off-market hours: Sleep and poll Telegram commands
                    time.sleep(10)
                    continue
